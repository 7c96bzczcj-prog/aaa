...
## N. v2.2 —— MDPC 定义错误（**归因规范**）与其后果

脚本 `scripts/v22_mdpc.py`，日志 `results/v22_mdpc.log`。

| # | 断言 | 证据类型 | 等级 | 出处 | 是否推翻初判 / 归因 |
|---|---|---|---|---|---|
| **N1** | **v2.1 §6.7 规则 2 的 MDPC 定义是错的：把"最小可检测比例变化"写成了"1 倍标准误 ÷ 基线"。1 倍标准误对应约 17% 的功效，不是"最小可检测"。** 同一个 XCL1 在三种口径下得 **23.9%（1×SE）/ 50.3%（t₀.₉₇₅）/ 67.1%（80% 功效）**——**规则 3 的判定完全取决于选哪个** | 条文分析 + 实测 | **[M]** | `results/v22_mdpc.csv` | **是 / 归因规范** |
| **N2** | **该缺陷在 v2.1 里造成过一次叙述与算术相互矛盾，且此前的修复很脆弱。** 手算 MDPC(XCL1) = 22.3% **小于** R1 的 ~23%，按规则 3 字面**允许写 `PRESERVED`**；叙述却写"不满足"。配对复算抬到 23.9%（> 23.6%）才使两者一致——**但判定只靠 0.3 个百分点**。按 v2.2 定义为 **67.1% 对 23.6%，相差近三倍，判定不再脆弱** | 实测 | **[M]** | 同上；`docs/V2.2_CHANGELOG.md` | **是 / 归因规范** |
| **N3** | **四基因 MDPC（α=0.05 双侧，功效 80%）**：CCL3 **25.0%**、CCL4 **18.9%**、CCL5 **23.3%**、**XCL1 67.1%**。对照基准 R1 观测 −23.6% → 仅 CCL4、CCL5 满足规则 3 | 实测 | **[M]** | 同上 | 否 |
| **N4** | **规则 3 需要一条补注，已写入 v2.2**：CCL3 的 MDPC（25.0%）高于基准，但它自身被检出下降（−27.1%，p = 0.0072）。**下降由检验确立，不由 MDPC 确立；规则 3 只约束"保留/不变"型陈述。** 无此补注会把 CCL3 误读为"测不出" | 条文分析 | **[M]** | 本报告 | **是 / 归因规范**（补条文空缺） |
| **N5** | **修正我自己此前的一处口径错误：先前报告的"技术底 / 供者间 SD = 0.06×–0.60×"是拿极差比标准差**，口径不一致，**高估**了技术占比，并被写进了 v2.1 修订案的算例。n=2 时 SD ≈ \|a−b\|/√2；同口径重算得 **0.13×–0.39×** | 实测 | **[M]** | `results/v22_attenuation.csv` | **是 / 归因执行**（我的报告口径） |
| **N6** | **衰减校正后近零相关依然近零，故它是设计事实而非测量误差。** 可靠度 **0.85–0.98**；校正后 CCL3 +0.18→+0.19~+0.21、CCL4 −0.07→−0.07、CCL5 −0.05→−0.05、XCL1 −0.09→−0.10。**修订案里"取 0.60× 则可靠度 ≈ 0.64、真实 r ≈ 0.28"的算例源于 N5 的口径错误，不成立** | 实测 | **[M]** | 同上 | **是 / 归因执行**（修正修订案算例） |
| **N7** | **尺度可行性：XCL1 需约 153 对配对供者才能检出与 R1 同比例（−23.6%）的变化**（现 n = 19；所需 SE 0.78 pp 对现有 2.23 pp）。CCL4（12）、CCL5（18）已足，CCL3 需 21。**根因是基线 0.09 太低，不是 n 不够**——而 GEO 重扫已确认 CD45⁺ + 配对 + ≥20 个体的公共数据集只有本数据集一个 | 实测 | **[M]** | `results/v22_mdpc.log` | 否 |
| **N8** | **对 §2 门顺序的后果（授权主计算前必须知情）**：即使 G-KILL-3 ① 全负、`GSE154826` 全过 §4，**§6 主计算也无法就 R2 得出任何结论；能确立的只有 R1 的下降。** 可行路径是换尺度——更深/靶向扩增、蛋白或 PrimeFlow、或换到 XCL1 丰度高的区室（蜕膜 NK 检出率 **0.756**，VT2018 `[M]`）。**Böttcher 测 `Xcl1` 用的正是 PrimeFlow，很可能是撞到了同一个分辨率下限** | 推论 | **[M]** | 本报告 | 否 |
